...
export default function UserProfileModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    if (user) {
      console.log("User from profile modal:", user);
      setName(user?.name || "");
    }
  }, [user]);
  ...
  return (
    <div className="fixed inset-0 bg-[#EFE7DE] coffee-scroll bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-[#F7F3EF] rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col relative">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-[#9B8D7F] hover:text-[#3F3A34] transition-colors z-10"
        >
        ...
        <div className="overflow-y-auto flex-1 p-6">
          ...
          <div className="space-y-4 bg-white rounded-xl p-4 shadow-sm">
            ...
            <div>
              ...
              />
              <p className="text-xs text-[#9B6258] mt-1">Email cannot be changed</p>
            </div>

            {/* Password section commented out because API doesn't exist */}
            {/*
            {isEditing && (
              <div className="pt-4 border-t">
                <p className="text-sm font-medium text-[#3F3A34] mb-3">Change Password (Optional)</p>
                ...
              </div>
            )}
            */}
            ...
            {!isEditing ? (
              <>
                <button
                  ...
                  className="w-full py-2.5 bg-[#6F4E37] text-white rounded-lg hover:bg-[#5A3E2B] transition-colors flex items-center justify-center gap-2 font-medium"
                >
                  Edit Profile
                </button>
                <button
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={handleSave}
                  disabled={loading}
                  className="flex-1 py-2.5 bg-[#6F4E37] text-white rounded-lg hover:bg-[#5A3E2B] transition-colors disabled:opacity-50 font-medium"
                >
...
//                 <button
//                   onClick={() => setIsEditing(true)}
//                   className="w-full py-2.5 bg-[#6D5DFB] bg-[#6F4E37] text-white rounded-lg hover:bg-[#5A3E2B] transition-colors flex items-center justify-center gap-2 font-medium"
//                 >
